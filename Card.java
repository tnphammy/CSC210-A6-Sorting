      ...
      for (int i = 0; i < ranks.length; i++) {
        for (int j = 0; j < suits.length; j++) {
          ...
          protoDeck[i+ranks.length*j].frontSide = 
            toolkit.getImage(new java.net.URI("http://cs.smith.edu/~nhowe/teaching/csc212/Graphics/" + id + ".png").toURL());
          mediaTracker.addImage(protoDeck[i+ranks.length*j].frontSide,i+ranks.length*j);
        }
      }
      ProtoCard.backSide = toolkit.getImage(new java.net.URI("http://cs.smith.edu/~nhowe/teaching/csc212/Graphics/b1fv.png").toURL());
